...
def api_manage_appointment():
    if 'user' not in session:
        return jsonify({"success": False, "message": "Unauthorized. Please log in as staff or superadmin."}), 401
...
def api_update_camp():
    if 'user' not in session or session.get('role') not in ['superadmin', 'staff']:
